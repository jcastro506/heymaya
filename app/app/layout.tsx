...
/** The thin UI shell (plan §7 S4): five tabs, mobile-first, the chat is elsewhere. */
    ...
    <div className="min-h-dvh max-w-md mx-auto flex flex-col">
      ...
      <nav className="fixed bottom-0 left-0 right-0 border-t border-white/10 bg-black/90 backdrop-blur" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
        <ul className="max-w-md mx-auto grid grid-cols-4">
          {TABS.map((t) => (
            <li key={t.href}>
              <Link href={t.href} className={`block text-center py-3 text-sm ${path?.startsWith(t.href) ? "text-emerald-400" : "opacity-60"}`}>{t.label}</Link>
            </li>
          ))}
        </ul>
      </nav>
