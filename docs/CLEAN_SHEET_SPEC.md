...
| ⭐ **`listCommentedPosts` as the `answer-people` work queue** (§2155) | **No wrapper exists.** `listInboxComments` · `listConversations` · `replyToComment` · `sendDm` · `igListComments` are wrapped; this one isn't. It's the better shape — *which of our posts have unanswered comments*, rather than paging the whole inbox — so it's worth adding, but the skill must not name it meanwhile. **Found by the skill-coherence test, after the first version of that test failed to catch it.** | 🟡 Skill points at the inbox listing; gap recorded |
| ⭐ **The 30-minute warm window** (§17.36.2) | **Not expressible in a Fly machine config.** `autostop` fires on idle and there is no configurable grace period, so the promise that "a back-and-forth conversation never pays a cold start twice" has no mechanism. Auto-stop itself IS wired (`autostop: "stop"`, `autostart`, `min_machines_running: 0`); only the grace period is missing. Needs Convex holding the machine open after an interaction. **Until then a multi-turn conversation may pay a cold start more than once** — which is exactly the case the typing indicator was sized for. | 🟡 Auto-stop shipped, warm window unimplemented |
