...
> **Not built:** the Zernio analytics merge into ownPosts (after the probe) · Apple Calendar · calendar push channels · the staging Convex deployment (the branch exists; the project needs a human) · fixture recording against real credits (the recorder is ready) · the deletion receipt email and PostHog person delete (vendor choices) · the chat-register golden set and the simulated-month test (need labels and credits).
