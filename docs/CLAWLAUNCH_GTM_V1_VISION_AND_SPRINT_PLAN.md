...
- **COGS / unit economics** — per-user Fly + daily LLM judgment passes + daily scraping (×handles×platforms) + selective Gemini video-watch. Stacking always-on daily features (E's watch cron, dynamic adjustment, continuous polling) risks costing more than the subscription. **Un-park COGS BEFORE pricing the tiers.** Bound frequency + selectivity on all always-on crons.
