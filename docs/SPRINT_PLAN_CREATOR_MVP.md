...
## Sprint 5 — New skills wave 1: picture-verifier + trend-watcher + idea-generator + transcribe — DONE

**Status:** Merged to `staging`. 2872/2872 tests, tsc clean, regression smoke green on real Fly.

**Outcome (4 parallel slices):**
- **`maya-picture-verifier`** (32 tests) — generates iMessage-shaped verification questions for low-confidence picture inferences. Citation-firewall composes with `maya-citation-firewall`'s `runFirewall` (two-layer: skill-local rule + broader firewall). Adversarial parser handles "yes!!!", "I guess maybe?", "ugh fine", emoji-only, prompt-injection inside reply. **Sibling drift flagged: `creatorPicture.needsVerification[]` field doesn't yet exist in `convex/schema.ts`** — orchestrator/Sprint 6 will add it when wiring the verifier into the onboarding flow.
- **`maya-trend-watcher`** (34 tests) — pulls cross-niche/niche-specific trend signal (popular_hashtags + trendingFeed + popular_creators), scores fit-to-creator (1-10) with the model, drops uncited or banned-topic-conflicting trends. Dedupe key: `${platform}:${kind}:${slug(trendPattern)}` — same hashtag on TikTok vs IG surfaces separately (correct: trend mechanics differ per platform).
- **`maya-idea-generator`** (30 tests) — composes against `voiceFingerprint`, last-30d posts, top hooks, trend candidates. Citation-validation = set-membership lookup against actual `recentPosts.map(p => p.id)` (not regex — catches hallucinated post IDs that look real, doubles as cross-tenant gate).
- **`maya-transcribe`** (36 tests) — wraps `theplasmak/faster-whisper@1.5.1` ClawHub pin. Model heuristic: ≤120s → small, 120-600s → medium, >600s → large-v3, unknown → medium. VAD off for clips <5s. Defensive zod parser fails closed on garbage JSON.

**Registry:** `BUNDLED_SKILLS` regenerated by `scripts/sync-bundled-skills.ts` — now ships 28 skills (1 scrapecreators-api + 23 prior maya-* + 4 Sprint 5 new). 132 new tests added, 0 regressions.

**Real-world bar:** All 4 skills are pure-logic libraries; their real-world output (verification round-trip, trend pull, idea cite, voice-memo transcribe) requires the Sprint 6 onboarding redesign + Sprint 7 channel/heartbeat wiring to fully exercise. The skill scripts + tests are the v0 deliverable; live demonstrations defer to Sprints 6-7.

**Carry-forward into Sprint 6:**
- Add `needsVerification[]` field to `creatorPicture` schema as part of Sprint 6's onboarding redesign.
- Wire `maya-picture-verifier` into the post-synthesis verify round-trip (the operator-facing iMessage flow).
- Wire `maya-trend-watcher` + `maya-idea-generator` into the heartbeat tick (per Sprint 3's HEARTBEAT.md spec).
- Wire `maya-transcribe` into the iMessage attachment handler (Sprint 7 channel work).

---

## Sprint 5 — New skills wave 1 (original spec, kept for reference)
