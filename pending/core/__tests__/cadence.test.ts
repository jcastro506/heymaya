...
describe("⭐ THE FOUNDER'S DAY, NOT UTC", () => {
  ...
  it("a streak counted in the founder's zone stays unbroken across the UTC boundary", async () => {
    ...
    const creatorId = await seed(t, "America/New_York");
    // Three consecutive ET evenings, each of which is the NEXT day in UTC.
    for (let i = 0; i < 3; i += 1) {
      await addPlacement(t, creatorId, {
        publishedAt: Date.UTC(2026, 7, 5, 0, 30, 0) - i * DAY,
      });
    }
    const r = await t.query(internal.core.cadence.cadence, { creatorId, now: NOW });
...
describe("⭐ WHAT COUNTS AS A PLACEMENT", () => {
  it("⭐ AN UNCONFIRMED PUBLISH DOES NOT COUNT", () => {
    ...
    return withSeed(async (t, creatorId) => {
      await addPlacement(t, creatorId, { publishedAt: NOW, linkStatus: "unknown" });
      const r = await t.query(internal.core.cadence.cadence, { creatorId, now: NOW });
  ...
  it("a dead link breaks the run rather than being papered over", () =>
    withSeed(async (t, creatorId) => {
      await addPlacement(t, creatorId, { publishedAt: NOW, linkStatus: "gone" });
      const r = await t.query(internal.core.cadence.cadence, { creatorId, now: NOW });
      expect(r.streak).toBe(0);
    }));
  ...
  it("⭐ SEVEN DAYS OF REPLIES IS NOT THE EXIT CRITERION", () =>
    ...
    withSeed(async (t, creatorId) => {
      for (let i = 0; i < 8; i += 1) {
        await addPlacement(t, creatorId, { publishedAt: NOW - i * DAY, kind: "reply" });
      }
      const r = await t.query(internal.core.cadence.cadence, { creatorId, now: NOW });
  ...
  it("two placements in one day is still one day", () =>
    withSeed(async (t, creatorId) => {
      await addPlacement(t, creatorId, { publishedAt: NOW });
      await addPlacement(t, creatorId, { publishedAt: NOW - 3600_000 });
      const r = await t.query(internal.core.cadence.cadence, { creatorId, now: NOW });
...
describe("⭐ THE MORNING PROBLEM", () => {
  it("⭐ AT 10AM WITH NOTHING POSTED YET, THE STREAK IS INTACT", () =>
    ...
    withSeed(async (t, creatorId) => {
      for (let i = 1; i <= 4; i += 1) {
        await addPlacement(t, creatorId, { publishedAt: NOW - i * DAY });
      }
      const r = await t.query(internal.core.cadence.cadence, { creatorId, now: NOW });
      expect(r.todayDone).toBe(false);
  ...
  it("a genuinely missed day breaks it, and says so", () =>
    withSeed(async (t, creatorId) => {
      ...
      await addPlacement(t, creatorId, { publishedAt: NOW - 2 * DAY });
      const r = await t.query(internal.core.cadence.cadence, { creatorId, now: NOW });
      expect(r.streak).toBe(0);
  ...
  it("the days list is the receipt behind the number", () =>
    withSeed(async (t, creatorId) => {
      await addPlacement(t, creatorId, { publishedAt: NOW, kind: "post" });
      await addPlacement(t, creatorId, { publishedAt: NOW - DAY, kind: "reply" });
      const r = await t.query(internal.core.cadence.cadence, { creatorId, now: NOW });
...
describe("⭐ CROSS-TENANT", () => {
  it("one founder's streak is never another's", async () => {
    ...
    const rb = await t.query(internal.core.cadence.cadence, { creatorId: b, now: NOW });
    expect(rb.streak).toBe(0);

    const fleet = await t.query(internal.core.cadence.fleetCadence, { now: NOW });
    expect(fleet).toHaveLength(2);
    expect(fleet.find((r) => r.creatorId === a)?.streak).toBe(5);
    expect(fleet.find((r) => r.creatorId === b)?.streak).toBe(0);
  });
});
...
async function withSeed(
  fn: (t: ReturnType<typeof convexTest>, id: Id<"creators">) => Promise<void>
) {
  const t = convexTest(schema, modules);
  const creatorId = await seed(t);
  await fn(t, creatorId);
}
...
async function seed(
  ...
  return await t.run(async (ctx) => {
    const clerkUserId = await ctx.db.insert("creators", {
