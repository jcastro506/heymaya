...
describe("INVARIANT 3 — a directive is never mutated", () => {
  ...
  it("superseding leaves the original text byte-for-byte intact", async () => {
    ...
    const first = await t.mutation(internal.maya.directives.append, {
      customerId,
      kind: "channel_toggle",
      verbatim: "stop posting on linkedin its dead",
      now: NOW,
    });
    const second = await t.mutation(internal.maya.directives.append, {
      customerId,
      kind: "channel_toggle",
  ...
  it("the new row points back at what it replaced", async () => {
    const t = convexTest(schema, modules);
    const customerId = await seed(t, "chain");
    const first = await t.mutation(internal.maya.directives.append, {
      customerId,
      kind: "timing_window",
  ...
  it("nothing is ever deleted — forgotten rules stay quotable", async () => {
    const t = convexTest(schema, modules);
    const customerId = await seed(t, "forget");
    const { directiveId } = await t.mutation(internal.maya.directives.append, {
      customerId,
      kind: "phrase_ban",
      verbatim: "stop saying game-changer",
      now: NOW,
    });
    await t.mutation(internal.maya.directives.forget, {
      customerId,
      directiveId,
      now: LATER,
    });
  ...
  it("CROSS-TENANT: you cannot supersede another account's rule", async () => {
    ...
    const theirs = await t.mutation(internal.maya.directives.append, {
      customerId: b,
      kind: "voice",
      verbatim: "B's rule",
      now: NOW,
    });
    const result = await t.mutation(internal.maya.directives.append, {
      customerId: a,
      kind: "voice",
  ...
  it("CROSS-TENANT: you cannot forget another account's rule", async () => {
    ...
    const result = await t.mutation(internal.maya.directives.forget, {
      customerId: a,
...
describe("the verbatim quote — what makes it worth storing", () => {
  it("keeps casing, typos and punctuation exactly as typed", async () => {
    const t = convexTest(schema, modules);
    const customerId = await seed(t, "exact");
  ...
  it("our interpretation lives in a separate field so the quote stays clean", async () => {
    const t = convexTest(schema, modules);
    const customerId = await seed(t, "interp");
    const { directiveId } = await t.mutation(internal.maya.directives.append, {
      customerId,
      kind: "channel_toggle",
...
describe("the three commands that always work", () => {
  it("'what rules are you following?' returns active rules, newest first", async () => {
    const t = convexTest(schema, modules);
    const customerId = await seed(t, "rules");
    for (const [i, text] of ["first rule", "second rule", "third rule"].entries()) {
      await t.mutation(internal.maya.directives.append, {
        customerId,
        kind: "voice",
        verbatim: text,
        now: NOW + i * 1000,
    ...
    const rules = await t.query(internal.maya.directives.activeRules, {
      customerId,
    });
  ...
  it("'forget that' takes it out of the active set but not the history", async () => {
    const t = convexTest(schema, modules);
    const customerId = await seed(t, "cmd_forget");
    const { directiveId } = await t.mutation(internal.maya.directives.append, {
      customerId,
      kind: "topic",
    ...
    await t.mutation(internal.maya.directives.forget, { customerId, directiveId });

    expect(
      await t.query(internal.maya.directives.activeRules, { customerId })
    ).toHaveLength(0);
    expect(
      await t.query(internal.maya.directives.history, { customerId })
    ).toHaveLength(1);
  });
  ...
  it("'why did you do X?' is answered from the stored history, not reconstructed", async () => {
    ...
    const t = convexTest(schema, modules);
    const customerId = await seed(t, "why");
    await t.mutation(internal.maya.directives.append, {
      customerId,
      kind: "timing_window",
      verbatim: "dont post before 9",
    ...
    const history = await t.query(internal.maya.directives.history, {
      customerId,
      kind: "timing_window",
    });
  ...
  it("rules are scoped per customer", async () => {
    const t = convexTest(schema, modules);
    ...
    expect(
      await t.query(internal.maya.directives.activeRules, { customerId: a })
    ).toEqual([]);
  });
});
...
describe("precedence — recency wins, but never silently", () => {
  it("the newest rule of a kind wins", async () => {
    const t = convexTest(schema, modules);
    const customerId = await seed(t, "precedence");
    await t.mutation(internal.maya.directives.append, {
      customerId,
      kind: "timing_window",
      verbatim: "dont post before 9",
    ...
    const { winning } = await t.query(internal.maya.directives.effectiveRule, {
      customerId,
      kind: "timing_window",
    });
  ...
  it("and hands back what it replaced, so it can be NAMED in a clause", async () => {
    ...
    const t = convexTest(schema, modules);
    const customerId = await seed(t, "named");
    const first = await t.mutation(internal.maya.directives.append, {
      customerId,
      kind: "timing_window",
      verbatim: "dont post before 9",
    ...
    const { supersededChain } = await t.query(
      internal.maya.directives.effectiveRule,
      { customerId, kind: "timing_window" }
    );
  ...
  it("walks a chain of three back to the original", async () => {
    const t = convexTest(schema, modules);
    const customerId = await seed(t, "deep_chain");
    let prev: Id<"directives"> | undefined;
    for (const [i, text] of ["9am", "10am", "11am"].entries()) {
      const res = await t.mutation(internal.maya.directives.append, {
        customerId,
        kind: "timing_window",
        verbatim: text,
        supersedesId: prev,
  ...
  it("kinds don't interfere — a voice rule never wins a timing question", async () => {
    const t = convexTest(schema, modules);
    const customerId = await seed(t, "kinds");
    await t.mutation(internal.maya.directives.append, {
      customerId,
      kind: "timing_window",
      verbatim: "dont post before 9",
  ...
  it("no rule of that kind is null, not a guess", async () => {
    const t = convexTest(schema, modules);
    const customerId = await seed(t, "none");
    const { winning, supersededChain } = await t.query(
      internal.maya.directives.effectiveRule,
      { customerId, kind: "cadence" }
    );
