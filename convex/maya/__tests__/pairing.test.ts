...
describe("claiming a pairing token", () => {
  it("binds the chat id", async () => {
    ...
    const customerId = await seed(t, "ok", {
      pairingToken: "tok_live",
      pairingExpiresAt: NOW + PAIRING_TTL_MS,
    });

    const result = await t.mutation(internal.maya.pairing.claimPairing, {
      token: "tok_live",
      chatId: "chat_99",
      now: NOW,
    });
  ...
  it("IS ONE-SHOT — a second claim on the same token fails", async () => {
    ...
    expect(
      (
        await t.mutation(internal.maya.pairing.claimPairing, {
          token: "tok_once",
          chatId: "chat_first",
          now: NOW,
        })
    ...
    const second = await t.mutation(internal.maya.pairing.claimPairing, {
      token: "tok_once",
  ...
  it("a second claim cannot REBIND an already-paired chat", async () => {
    const t = convexTest(schema, modules);
    const customerId = await seed(t, "rebind", {
      pairingToken: "tok_r",
      pairingExpiresAt: NOW + PAIRING_TTL_MS,
    });
    await t.mutation(internal.maya.pairing.claimPairing, {
  ...
  it("ONE CUSTOMER'S TOKEN NEVER PAIRS ANOTHER'S ACCOUNT", async () => {
    ...
    const other = (await t.run((ctx) => ctx.db.get(theirs))) as Doc<"customers">;
    expect(other.telegramChatId).toBeUndefined();
    const own = (await t.run((ctx) => ctx.db.get(mine))) as Doc<"customers">;
...
describe("the image the machine boots", () => {
  it("is NOT the tag that doesn't exist", async () => {
    // The first live deploy failed with MANIFEST_UNKNOWN: unknown tag=latest.
    // `:latest` was assumed rather than checked against what v1 actually ships.
    const { DEFAULT_IMAGE } = await import("../setup");
    expect(DEFAULT_IMAGE).not.toMatch(/:latest$/);
  });

  it("is pinned by digest, in the tag@digest form Fly requires", async () => {
    // The digest makes it immutable — the registry app was once destroyed by a
    // fleet sweep, taking the image with it. And Fly's Machines API rejects a
    // bare @digest identifier; it needs the tag too.
    const { DEFAULT_IMAGE } = await import("../setup");
    expect(DEFAULT_IMAGE).toMatch(/^registry\.fly\.io\/[\w-]+:v[\d.]+@sha256:[0-9a-f]{64}$/);
  });
});
