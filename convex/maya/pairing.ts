 ...
 * type v2 customers don't have and shouldn't. Rather than mint a fake agent row
 * to satisfy a foreign key, the token lives on the customer, which is the thing
 * being paired.
 *
...
export const createPairingLink = mutation({
  ...
  handler: async (ctx): Promise<PairingLink> => {
    ...
    if (!customer) return { ok: false, error: "tell me about your product first" };
    ...
    const now = Date.now();
    const live =
      customer.pairingToken &&
      customer.pairingExpiresAt &&
      customer.pairingExpiresAt > now;

    const token = live ? customer.pairingToken! : mintToken();
    const expiresAt = live ? customer.pairingExpiresAt! : now + PAIRING_TTL_MS;
...
export const claimPairing = internalMutation({
  ...
  handler: async (
    ...
    if (!customer) return { paired: false, reason: "that link isn't valid" };
    if (!customer.pairingExpiresAt || customer.pairingExpiresAt <= now) {
