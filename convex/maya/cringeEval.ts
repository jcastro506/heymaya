...
/**
 * ⚠️ `runEvalOnPublished` WAS DELETED HERE (2026-08-18). It read `placements`
 * rather than `drafts`, arguing that measuring drafts would grade the work
 * instead of the product and would include writing the founder rejected.
 *
 * That argument was right and is already satisfied: `herWriting` reads
 * placements first AND skips `outcome === "rejected"` drafts, so the wired path
 * (`runWeeklyEval` → `runEvalForCustomer` → `runEval`) already measures only
 * what a stranger could have seen. It was a third entry point onto the same
 * work, and three doors make a surface look larger than it is.
 */
