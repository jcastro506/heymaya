...
export const SERVICE_STANDING_ORDERS: ReadonlyArray<ServiceStandingOrderProgram> = [
  {
    ...
    description:
      "Daily 7am push: today's schedule + overnight reviews + fresh leads, cited; folds in the GBP SEO auditor's score + top nudge for the brief.",
    scope:
      "Read today's `serviceJobs` schedule + overnight `reviews` + last-12h `inboundLeads`; **also run the GBP SEO auditor** via `convex/gbp/computeHealthScore.ts#runGbpHealthAuditForBusiness` (judgment-driven — Maya scores local-pack health 0-100 + drafts 1-3 nudges + enqueues them as `mayaTaskQueue.kind='gbp.seo.nudge'` rows). Assemble a <200-word brief with one cited insight + one action + any pending approvals + the auditor's score + her top nudge; write `dailyBriefs`; push to operator's primary channel. The auditor pulls fresh `wikiLearnings` from `concepts/what-works/gbp/*` so the score reflects what's actually driven outcomes for THIS business.",
    triggers: "Cron `morning_brief` at 7:00am operator-tz.",
    approvalGates: "None — the brief is a push. Referenced actions carry their own gates. Auditor nudges with `suggestedAction='maya-queue'` may invoke downstream skills directly (e.g. `gbp-post-optimizer` for unused photos when Maya judges cadence is light); `'maya-draft'` and `'operator-fix'` queue for approval.",
    escalation:
      "If GBP/Zernio API is down, ship the brief without the review section and flag inline ('Google reviews offline this morning — will retry on next heartbeat'). Never invent metrics. If the auditor's LLM call fails, the safe-fallback `gbpHealthScores` row (score=50, reasoning='Maya did not have an LLM available') ships and the brief omits the SEO line.",
    cronMessage:
      "Run morning brief: pull today's schedule + overnight reviews + fresh leads; run `runGbpHealthAuditForBusiness` for fresh score + nudges; draft brief with one cited insight + one action + pending approvals + the auditor's score, write `dailyBriefs`, push to primary channel. Stay under 200 words.",
  },
  {
  ...
  },
  {
    ...
    description:
      "Sundays 9am: weekly digest of named competitor GBP changes; afterwards (≈Sun 10pm via the same cron tail) the outcome poller refreshes engagementMetrics for the prior week's gbpPosts, the learnings extractor synthesizes patterns into memory-wiki concepts/what-works/*, AND a deeper-thinking GBP SEO audit runs (judgment over the freshly-synthesized wiki + competitor digest).",
    scope:
      "Sundays 9am op-tz: ScrapeCreators sweep of `localPositioning.namedCompetitors[]` GBPs. Run `maya-service-competitor-watcher` for a weekly digest: rating changes, new posts, promo prices. Then nightly (≈Sun 10pm op-tz, same standing-order tail): trigger the outcome polls — `convex/outcomes/gbpInsightsPoller.ts#pollGbpInsightsForBusiness` (refreshes `gbpPosts.engagementMetrics` for the trailing 30d) and `convex/outcomes/metaInsightsPoller.ts#pollMetaInsightsForBusiness` (FB Pages messages count) — and run `maya-service-learnings-extractor` via `convex/agents/packs/maya_service/runLearningsExtractor.ts#runLearningsExtractor`. The extractor reads the now-fresh outcome chain (gbpPosts.engagementMetrics → inboundLeads.originatingActionId → serviceJobs.originatingLeadId, plus reviewRequests.responseRating from `convex/outcomes/attribution.ts`) and writes a `weeklyLearnings` row + `concepts/what-works/<platform>/*` wiki pages (sample-size ≥ 3, outcome-attributed only). **After the wiki has been refreshed, run the weekly GBP SEO audit** via `convex/gbp/computeHealthScore.ts#runGbpHealthAuditForBusiness` — Maya now has the freshest `concepts/what-works/gbp/*` to weight her judgment by. The weekly audit is judgment-driven, not threshold-rule-driven (operator directive 2026-04-27): she looks at the bundle holistically and produces score + reasoning + nudges. All polls + extraction + audit run at every tier — outcome learnings are the moat. Fold competitor digest + Sunday-night SEO audit summary into Monday's morning brief; the wiki pages compound automatically.",
    ...
    cronMessage:
      "Run competitor watch: ScrapeCreators sweep of named competitor GBPs, call `maya-service-competitor-watcher` for digest, fold into Monday morning brief. Same standing order tail (≈Sun 10pm op-tz): poll `gbpPosts.engagementMetrics` via `gbpInsightsPoller.pollGbpInsightsForBusiness` and Meta Pages via `metaInsightsPoller.pollMetaInsightsForBusiness`, run `runLearningsExtractor.runLearningsExtractor` to synthesize `weeklyLearnings` + `concepts/what-works/*` wiki pages, then run `runGbpHealthAuditForBusiness` for a weekly judgment-driven SEO audit weighted by the now-fresh wiki learnings.",
  },
  {
    id: "revenue_snapshot",
