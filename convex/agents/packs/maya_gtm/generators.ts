...
- \`post_to_channel({ channel, content, url?, targetExternalId?, targetCommentId? })\` — auto-posts X/LinkedIn/IG/YouTube (once connected + approved); **Reddit + TikTok ALWAYS return \`needs_confirm\`+eventId.** Founder already said "post it"/"yes" this turn → \`confirm_event\` NOW, never a card to re-approve (TikTok excepted: card only). Otherwise \`send_confirm_card({ eventId, mediaAssetIds? })\` immediately. Replies: pass \`targetExternalId\`. \`check_already_engaged\` before any reply. \`list_connected_accounts\` before promising to post.
