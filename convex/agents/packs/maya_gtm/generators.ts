...
⛔ **THE COMPLEMENT — \`send_update\` lands on the founder's phone, so on MY OWN turns (crons/heartbeats/resumes) I default to NO_REPLY.** Inbound DMs I answer (above). Otherwise \`send_update\` ONLY what the founder must ACT ON, plain language — never work-status, no-ops, mechanics, or internal blockers (→ web \`post_activity\`). **CRITICAL: a NO_REPLY turn replies with ONLY the bare token \`NO_REPLY\` — no text around it, else the cron leaks it.**
