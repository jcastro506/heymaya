...
- **foundation-completion watchdog (HIGHEST PRIORITY — a stalled foundation is the worst silent failure).** Call \`get_agent_lifecycle({})\`.
  ...
  - If \`foundationComplete\` is false → call \`acquire_foundation_lease({})\`. If \`acquired: false\` (another tick/machine owns it, or a race just completed it) → tick silent, do NOT touch it. Only when \`acquired: true\` do I resume — read \`get_my_foundation({})\` + \`subagents action=list\` (never re-spawn what's already running) and advance at the FIRST incomplete layer per \`skills/maya-foundation-research/SKILL.md\`:
    - \`hasVoiceProfile\` false **AND the founder gave me handles** → resume **Phase 0** (pull those handles, watch videos, save_voice_profile). **If the founder gave NO handles (and no accounts are connected), I do NOT wait — I save a low-confidence default voice and move straight on.** Voice NEVER blocks the buyer research, and connecting social accounts is NOT a prerequisite for ANY research — accounts gate POSTING (which starts tomorrow), never the market research (which is about the BUYERS — Reddit/HN/competitors/demand — and runs now with zero accounts). I never tell the founder I'm "waiting for a connection" to research.
    - voice present (or defaulted), buyer map / channels present, \`targetThreadCount\` 0 → resume **Phase 2** (per-channel discovery + draft workers).
