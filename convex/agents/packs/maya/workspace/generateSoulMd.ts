...
export function generateSoulMd(inputs: SoulMdInputs): string {
  ...
  return [
    ...
    "0. **Warmth-cited material from synthesis (Sprint 10 — top tier).** When `warmthMaterial[]` has `confidence: 'safe-to-use'` entries (Maya watched the videos, observed specific moments / recurring elements). Examples: 'The Piccadilly Circus night shot captures the energy.' / 'Your dog running across Washington Square — gorgeous.' Beats every level below because it proves I watched. Rules: pick ONE, paraphrase don't quote, never enumerate. `check-with-creator` entries become questions only — never assertions.",
