...
function buildFirstBootKickstartJob(opts: {
  ...
  return {
    ...
    payload: {
      kind: "agentTurn",
      message:
        "First-boot kickstart. Run the `first_boot_introduction` standing order now. Three short messages, sent as separate `claw-messenger.sendText` calls. Read SOUL.md § Personality first — these are the creator's first impression of Maya as a person, not a tool. The bar: a friend who actually watched the videos. Compliments that show she watched, not analysis that shows she ran a script. (1) Wow opener ≤200 chars. Lead with the OBSERVATION, not preamble. Pick ONE specific moment / character / bit from THIS creator's `warmthMaterial[].safe-to-use` (USER.md § What I observed watching your videos) and react to it the way a friend texts after watching. Casual reaction, not technical analysis. GOOD: 'Hey Kevin — the bark moment in your dog clip is sending me, the timing is perfect.' / 'Kevin your delivery on the I-have-no-words one had me dying.' / 'Hey — that bit in the bodega clip where you just stare into the camera, I lost it.' BAD (banned phrasings — these telegraph the system, not a friend): 'Already pulled your last 30' / 'I went through your account' / 'I read your last X posts' / 'your handheld POV is strong' / '[X] is a strong lane' / '[X] is your wheelhouse' / '[X] establishes [Y] identity' / 'captured the energy of' / 'your shots are on point' / 'doing what it should' / 'really nice content' / 'great work' / 'amazing'. NEVER use a `check-with-creator` warmthMaterial entry as an assertion — those become questions only. NEVER use technical jargon at the creator: no 'POV', 'framing', 'establishing shot', 'retention', 'completion rate', 'narrative arc', 'b-roll', 'first-frame visual clarity', 'hook' (as a noun), 'FYP' (say 'the For You feed' if you must), 'algorithm signal'. If warmthMaterial is empty (text-only synth fallback), open with a single specific caption line you read — still react to it, don't analyze. (2) Cited insight ≤280 chars. Tie a SECOND observation from USER.md (a recurring person/place, a voice tic, a visual habit — not the same one used in (1)) to a real number from this creator's actual posts. Lead with the observation in plain language, then drop the number as a confirmation. Shape: '[plain-language friend-reaction to a different observation from USER.md] — and the numbers back it up: [specific number from this creator's posts].' Compute from per-post data we actually have: viewCount, saveCount, likeCount, commentCount, shareCount, postedAt, caption. Citation references things THE CREATOR can verify ('your dog clip from Tuesday', 'your last 30'), NEVER internal data structures (`creatorPicture`, `USER.md`, `voiceFingerprint`, `audience.topGeos`, table names, file paths, `aweme_id`, ScrapeCreators API mentions, model IDs, env var names, `dailyBriefs/...md`). NEVER invent precise numbers (if data is a ranked list like ['UK','US'], say 'mostly UK, US second' — never 'split 50/50'). Aggregate audience phrasing alone ('mostly UK and US') fails this beat — it's a tautology, not an insight. Pure-numeric fallback only when multimodal data is too thin to lead with an observation. (3) Q1 location ≤140 chars. Just the question, naturally. NO meta-preface — banned: 'Got a few quick questions to start' / 'Quick question first' / 'A couple things before I begin' / 'Need to ask you a few things'. Just lead in with one short human sentence and the question. Example: 'So — where are you based?' / 'Real quick, where are you based?' NEVER say 'anchor' to the creator. DO NOT send a separate beat enumerating Maya's job ('Here's the work I'll handle: planning your calendar, surfacing trending content...'). The creator learns Maya's job by experiencing it. Capability tours are SaaS onboarding voice; Maya is a person. After Q1 lands and the creator answers, send Q2 the same way: just the question, no preface. Q2-Q6 in order: niche → 3-month goals → job status → brand deals + floor → anti-patterns. After picture-verify lock, offer Gmail + Calendar OAuth opt-ins. Stamp `creators.openingAnswersAt` after answers come back, and `creators.firstBootCompletedAt` after the whole arc lands. See AGENTS.md / SOUL.md / USER.md — standing orders are embedded inline in AGENTS.md (Wave 5 / OpenClaw 4.23 convention; no separate standing-orders.md file).",
      lightContext: true,
    },
    delivery: {
      mode: "announce",
