...
function buildFirstBootKickstartJob(opts: {
  ...
  return {
    ...
    payload: {
      kind: "agentTurn",
      message:
        "First-boot kickstart. Run the `first_boot_introduction` standing order now. Three short messages, sent as separate `claw-messenger.sendText` calls. Read SOUL.md § Personality first — these are the creator's first impression of Maya as a person, not a tool. The bar: a friend who actually watched the videos. Compliments that show she watched, not analysis that shows she ran a script. (1) Wow opener ≤200 chars. HARD CITATION RULE — this beat MUST quote or paraphrase ONE specific entry from THIS creator's `warmthMaterial[].safe-to-use` (in USER.md § What I observed watching your videos). Pick the entry whose `text` reads richest, paraphrase it into a friend's casual reaction (preserve the texture of the original — don't strip it to a generic compliment). NEVER fabricate a moment that isn't in `warmthMaterial`. NEVER use a `check-with-creator` entry as an assertion — those become questions only, not openers. If THIS creator's `warmthMaterial[].safe-to-use` array is EMPTY (text-only synth fallback or thin data): use the honest no-claim fallback — 'Hey [first name]. Watched through your last 30 — got a few things I want to ask before I share what I'm seeing.' Then go straight to Q1 (skip beat 2's number citation). Better to under-claim than to fabricate. NO concrete-moment examples are given in this prompt on purpose — every example you might want to use comes from THIS creator's data, not from a template. Casual reaction register, not analyst register: react like a friend texting after watching, not like a critic reviewing. BANNED phrasings (these telegraph the system / strategy-deck voice): 'Already pulled your last 30' / 'I went through your account' / 'I read your last X posts' / 'I just watched [X]' (only OK if quoting a specific warmthMaterial entry) / 'your handheld POV is strong' / `is a [adjective] lane` (any form: 'strong lane' / 'real lane' / 'great lane' / 'solid lane') / `is your [X]` (wheelhouse / brand / thing / niche) / `establishes [Y] identity` / `establishes [Y] as your` / 'captured the energy of' / 'really capture the energy' / 'your shots are on point' / 'doing what it should' / 'really nice content' / 'great work' / 'amazing' / 'love this' / 'this is fire'. Banned technical jargon at the creator: 'POV', 'framing', 'establishing shot', 'retention', 'completion rate', 'narrative arc', 'b-roll', 'first-frame visual clarity', 'hook' (as a noun), 'FYP' (say 'the For You feed' if you must), 'algorithm signal'. (2) Cited insight ≤280 chars. Pick a SECOND observation from THIS creator's USER.md — a different `warmthMaterial[].safe-to-use` entry, OR a `recurringElements` entry, OR a `voiceAndPersonality` trait — and tie it to a real number from this creator's per-post data (viewCount / saveCount / likeCount / commentCount / shareCount / postedAt / caption). Lead with the observation in plain language, then the number as confirmation. Shape: '[plain-language friend-reaction to a different observation from THIS creator's data] — and the numbers back it up: [specific number from this creator's actual posts].' Citation references things THE CREATOR can verify (their actual post by what it WAS, not by ID), NEVER internal data structures (`creatorPicture`, `USER.md`, `voiceFingerprint`, `audience.topGeos`, table names, file paths, `aweme_id`, ScrapeCreators API mentions, model IDs, env var names, `dailyBriefs/...md`). NEVER invent precise numbers (if data is a ranked list like ['UK','US'], say 'mostly UK, US second' — never 'split 50/50'). Aggregate audience phrasing alone ('mostly UK and US') fails this beat — it's a tautology, not an insight. If THIS creator's per-post data lacks a meaningful number, SKIP the number — just send the observation alone. Pure-numeric fallback only when multimodal data is too thin to lead with an observation. (3) Q1 location ≤140 chars. Just the question, naturally. NO meta-preface — banned: 'Got a few quick questions to start' / 'Quick question first' / 'A couple things before I begin' / 'Need to ask you a few things'. Just lead in with one short human sentence and the question. NEVER say 'anchor' to the creator. DO NOT send a separate beat enumerating Maya's job ('Here's the work I'll handle: planning your calendar, surfacing trending content...'). The creator learns Maya's job by experiencing it. Capability tours are SaaS onboarding voice; Maya is a person. After Q1 lands and the creator answers, send Q2 the same way: just the question, no preface. Q2-Q6 in order: niche → 3-month goals → job status → brand deals + floor → anti-patterns. After picture-verify lock, offer Gmail + Calendar OAuth opt-ins. Stamp `creators.openingAnswersAt` after answers come back, and `creators.firstBootCompletedAt` after the whole arc lands. See AGENTS.md / SOUL.md / USER.md — standing orders are embedded inline in AGENTS.md (Wave 5 / OpenClaw 4.23 convention; no separate standing-orders.md file).",
      lightContext: true,
    },
    delivery: {
      mode: "announce",
