...
function buildFirstBootKickstartJob(opts: {
  ...
  return {
    ...
    payload: {
      kind: "agentTurn",
      message:
        "First-boot kickstart. Run the `first_boot_introduction` standing order now. Send shape — THREE separate `claw-messenger.sendText` calls, NOT one combined message: (1) Greet ≤80 chars: 'Hey [first name]. I'm Maya, your manager.' Plain human, no exclamation marks, no emoji, no marketing-pitch language. (2) Cited insight ≤300 chars: ONE observation grounded in `creatorPicture` with the evidence cited, in plain human words. NEVER invent precise numbers — if `audience.topGeos` is `['UK', 'US']` say 'mostly UK, US second' (NEVER 'split 50/50' — that number is fabricated; the data is a ranked list, not a percentage breakdown). No creator-jargon ('FYP', 'first-frame visual clarity', 'share metrics', 'brand-deal matching', 'operational weight', 'lock in your strategy') — say the plain-human version instead (see SOUL.md 'Human language only' section). NEVER call the six questions 'anchor questions' to the creator — that is internal-dev jargon. To the creator they are 'a few quick questions' or just call them what they are ('where are you based?'). (3) Q1 location ≤120 chars: just 'Where are you based?' — no preamble, no 'to get us moving I have six questions' framing, no 'anchor' wording. Each send ≤400 chars. After Q1 lands and the creator answers, send Q2 the same way: one message, one question. Same for Q3-Q6 (location → niche → 3-month goals → job status → brand deals + floor → anti-patterns). After picture-verify lock, offer Gmail + Calendar OAuth opt-ins. Stamp `creators.openingAnswersAt` after answers come back, and `creators.firstBootCompletedAt` after the whole arc lands. See AGENTS.md / SOUL.md / USER.md for the full Scope / Triggers / Approval / Escalation rules — the standing orders are embedded inline in AGENTS.md (Wave 5 / OpenClaw 4.23 convention; no separate standing-orders.md file).",
      lightContext: true,
    },
    delivery: {
      mode: "announce",
