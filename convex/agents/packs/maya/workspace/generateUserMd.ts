...
export function generateUserMd(inputs: UserMdInputs): string {
  ...
  return [
    ...
    `- **Plan:** ${plan}`,
    "",
    "## How I persist state (curl recipe — copy + fill body)",
    "",
    "Every state-bearing answer the creator gives me MUST land on the matching `lc_maya/*` endpoint via this exact pattern. The creatorId above goes in `creatorId`. The webhook secret is in env var `WEBHOOK_INTERNAL_SECRET`. The base URL is in env var `MAYA_CONVEX_HTTP_BASE`. Run via my `exec` tool.",
    "",
    "```",
    `curl -fsS -X POST "$MAYA_CONVEX_HTTP_BASE/lc_maya/submit_opening_answers" \\`,
    "  -H 'content-type: application/json' \\",
    `  -d '{"secret":"'"$WEBHOOK_INTERNAL_SECRET"'","creatorId":"${creator._id}", <FIELDS>}'`,
    "```",
    "",
    "Per-question field shapes (see TOOLS.md § 1 for full schema):",
    "- Q1 location → `\"locationCity\":\"...\",\"locationState\":\"...\",\"locationCountry\":\"...\",\"timezone\":\"...\"`",
    "- Q2 niche → `\"nicheInOwnWords\":\"...\"`",
    "- Q3 3-month goals → `\"goals3Mo\":\"...\"`",
    "- Q4 job status → `\"jobStatus\":\"full-time-creator\"|\"transitioning-full-time\"|\"side-hustle\"|\"hobby\"`",
    "- Q5 brand deals → `\"dealsInterest\":\"yes\"|\"maybe\"|\"no\",\"dealsFloorUsd\":<number>` (floor optional)",
    "- Q6 anti-patterns → `\"antiNiches\":[\"topic1\",\"topic2\"]` (empty array if none)",
    "",
