...
export function generateHeartbeatMd(_inputs: HeartbeatMdInputs): string {
  ...
  const lines = [
    ...
    "5. **Niche + trend scan** (6h cd). → `trendObservations`. No push; fold into brief.",
    "6. **Competitor pull** (6h cd). Each named peer's 24h → `competitorObservations`. Fold into brief.",
    "7. **Comment triage** (6h cd). Last 5 posts → `commentTriage`. Surface qs in brief.",
    "8. **Calendar peek** (12h cd; if connected). 1-14d → `maya-calendar-classifier`; relevant → propose arc.",
    "9. **Opportunity scout** (12h cd). UGC marketplaces + creator-call hashtags + local Brave; dedupe `opportunityScoutSeen`; top 3 to brief.",
