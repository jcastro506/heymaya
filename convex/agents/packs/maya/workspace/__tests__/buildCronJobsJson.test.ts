...
function creator(plan: "starter" | "pro" | "studio", tz = "America/Los_Angeles") {
  return { plan, timezone: tz };
}

describe("buildCronJobsJson", () => {
  it("starter receives only 'all'-tier cron programs (no Pro+ entries)", () => {
    const { jobs } = buildCronJobsJson({ creator: creator("starter") });
  ...
  it("pro receives the full cron set", () => {
    const { jobs } = buildCronJobsJson({ creator: creator("pro") });
    const ids = jobs.map((j) => j.entryId);
  ...
  it("studio receives the same set as pro (cron coverage is identical; cadence differences are out of scope here)", () => {
    const pro = buildCronJobsJson({ creator: creator("pro") }).jobs;
    const studio = buildCronJobsJson({ creator: creator("studio") }).jobs;
  ...
  it("every emitted job carries the creator's timezone", () => {
    const { jobs } = buildCronJobsJson({
      creator: creator("pro", "Asia/Tokyo"),
    });
    for (const j of jobs) {
  ...
  it("rejects an invalid IANA timezone", () => {
    expect(() =>
      buildCronJobsJson({ creator: creator("pro", "not a tz") })
  ...
  it("is deterministic across calls", () => {
    const a = buildCronJobsJson({ creator: creator("pro") });
    const b = buildCronJobsJson({ creator: creator("pro") });
