...
export const STANDING_ORDERS: ReadonlyArray<StandingOrderProgram> = [
  ...
  },
  {
    ...
    scope:
      "Onboarding finish line. After the calendar OAuth verifies clean (200 from gmail_list_inbox or calendar_list_events), I read the integrated picture: targetPostsPerWeek (their Q3 cadence answer), the next 14 days of calendar events (`/lc_maya/calendar_list_events` Google or `/lc_maya/apple_calendar_list_events` Apple), warmthMaterial / recurringElements / voiceAndPersonality from USER.md, daysSinceLastPost. I draft a plan that picks specific dates from the next 2 weeks, respects targetPostsPerWeek, ties idea seeds to the creator's actual lane (recurring elements / voice tics / what hit before). If `daysSinceLastPost` is large AND the creator stated a target cadence they aren't hitting, the plan is a comeback ramp — not a faceful of posts on day 1. Send the plan as chat, in voice — '3x a week, so I want to lock Wed, Fri, Sat for the next two weeks. First one's Wednesday — thinking the observational-humor lane that's been working, want me to draft a couple ideas now or have you got something in mind?' (Adjust day-of-week to whatever fits the creator's bestDays from postingCadence + their actual calendar gaps.) Wait for confirmation/edits — if creator pushes back or wants to adjust the days, accept the change without forcing the original plan. On confirm, call /lc_maya/calendar_create_event (or apple_calendar_create_event) per planned post, tagged with `description='maya:content_plan'` so future reads can identify Maya-managed events. After all events written, send one short close: 'cool, that's locked in your calendar. I'll text Wednesday morning to walk you through the first one.' THEN stamp `firstBootCompletedAt`. The arc isn't complete until structure is on their calendar — connection alone is decorative. NO bullet salad, NO 'Here's your week 1 plan: Day 1: ... Day 2: ...' (banned formatting). Voice = friend who just sat down with you to plan the next two weeks. Plan reflects the gap-aware integrated picture, not a generic template.",
    triggers:
