...
export const STANDING_ORDERS: ReadonlyArray<StandingOrderProgram> = [
  {
    ...
    // bug.)
    scope:
      "ONCE: 4 separate `claw-messenger.sendText` sends (greet + cited insight + scope list + Q1), then Q2-Q6 one at a time as creator answers. CRITICAL — after EACH answer arrives, BEFORE sending the next question, I MUST call `POST /lc_maya/submit_opening_answers` with the parsed field(s) for that answer (Q1 → locationCity+locationState+locationCountry+timezone; Q2 → nicheInOwnWords; Q3 → goals3Mo; Q4 → jobStatus; Q5 → dealsInterest+dealsFloorUsd?; Q6 → antiNiches). Use the curl pattern in TOOLS.md § 1. Without this call, `creators.openingAnswersAt` never stamps, the standing-order trigger stays active, and on the next inbound I will re-greet the creator from scratch — they will hate that. After Q6 lands and the final submit_opening_answers call returns ok: I post the picture summary + 1-3 verification questions drawn from `creatorPicture.needsVerification` (already populated by the synth that ran at deploy). Wait for confirmation. Apply any corrections via `POST /lc_maya/lock_picture` (corrections array, or empty for clean-confirm). The lock stamps `pictureLockedAt`. THEN — and only then — Gmail + Calendar OAuth offers via `start_oauth` (one message per provider, opt-in framing, never inline with questions). NEVER ask the meta-tone-question (\"supportive / strategic / tough-love\") — Sprint 6 deleted that beat; tone is calibrated FROM the answers. NEVER use \"anchor\" / \"anchor questions\" wording to the creator (internal-dev only). The 6 questions in target order: location → niche → 3-month goals → job status → brand deals + floor → anti-patterns. NO brand-deal floor pressure on first boot beyond Q5; floor calibration happens later when a real brand email lands. Stamp `firstBootCompletedAt` (via `lc_maya.update_creator`) after the OAuth offers ship — that's the arc-complete signal. See `playbook.md § 4.5` for the full send-shape spec + voice rules + anti-fabrication / no-jargon / no-internal-name rules. Read SOUL.md § Personality before composing — these are the creator's first messages from Maya the person, not Maya the tool.",
