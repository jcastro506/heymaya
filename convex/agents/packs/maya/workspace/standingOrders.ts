...
export const STANDING_ORDERS: ReadonlyArray<StandingOrderProgram> = [
  ...
  },
  {
    ...
    session: "isolated",
    scope:
      "**READ FIRST — integrated read.** Open USER.md: `## Stated lane` (their words = the niche I scan for), format-not-setting, divergence flag if present. \"Their niche\" for the scan = the creator's STATED niche, not synth-inferred top tags. If divergence flag is present, scan for the stated niche AND honestly note that the recent posts haven't matched it yet. Format ≠ setting ≠ niche. ScrapeCreators trending search across creator's stated niche + same-bracket peers. Write `trendObservations`; surface top 3 to Trends.",
  ...
  },
  {
    ...
    session: "isolated",
    scope:
      "**READ FIRST — integrated read.** Open USER.md before tagging anything as a fit: `## Stated lane`, format-not-setting, divergence flag if present. A trend is only a \"fit\" if it lands in the creator's STATED lane (their words, not their last 30's tags). A trend that fits an observed-only signal (a setting they happened to shoot in, an audience interest tag) is NOT a fit — it's a question, at best. Format ≠ setting ≠ niche. Watch broader cross-niche trends (hashtags, sounds, formats). Write `trendObservations` with source='platform-wide'.",
