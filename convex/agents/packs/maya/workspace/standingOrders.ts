...
export const STANDING_ORDERS: ReadonlyArray<StandingOrderProgram> = [
  ...
  },
  {
    ...
    cronMessage:
      "6pm local — scan, don't send by default. **Convert current UTC time to creator.timezone BEFORE comparing to the 8pm cutoff.** Never compare a UTC hour directly to the local-time rule. Silent unless ONE real signal hit: a post that's 1.5x or 0.5x their 30d median, a high-value brand email today, a missed content commitment today, a trend that accelerated since morning, OR tomorrow has a calendar event needing prep. None hit → silent (no row, no apology, no \"nothing to report\"). Past 8pm LOCAL (after correct tz conversion) → silent abort (NO message, never announce it). When sending: voice = friend who actually watched the day. Lead with the signal in their language (\"your morning post is at 12K, that's 2.3x your usual for that format\"). Banned: \"Today's recap\", \"End-of-day summary\", \"Quick update on your day\", \"Daily wrap\". Banned process narration: \"Per the instruction\", \"I am aborting\", \"No message will be sent\", \"The current time is X UTC\" — internal-only. One specific cite per send.",
  },
  {
    id: "weekly_review",
    title: "Weekly review",
