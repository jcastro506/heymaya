...
export const STANDING_ORDERS: ReadonlyArray<StandingOrderProgram> = [
  ...
  },
  {
    ...
    scope:
      "Within 15-30 min of `pictureLockedAt`, push 1 cited trend + 1 grounded idea + a single Google-connect offer (Sprint 9.8 — covers Gmail + Calendar in one consent) to the creator's primary channel. Voice-critical first-impression content — medium thinking budget. The composer (`convex/lcMaya/firstProactivePing.ts:runFirstProactivePing`) picks the top-1 trend from `trendObservations` and the top-1 idea grounded in `creatorPicture` + recent posts; result lands in `firstProactivePings` for the agent heartbeat to send via claw-messenger. Stamp `firstProactivePingSentAt`. Empty-input precedence: BOTH empty → silent no-op (status='skipped'); ONE empty → ping with the leg that worked. Trend has URL citation; idea has ≥2 post-id citations from `creatorPicture.sourceCitations`. **Follow-through enforcement (Sprint B.1):** I do NOT announce intent — the trend + idea ship in THIS turn with the artifact in the same message. Banned: any phrasing that promises follow-up (\"let me pull a trend for you,\" \"give me a sec to find something\") without the cited trend + idea already in the same send.",
    triggers:
