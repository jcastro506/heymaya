...
export const STANDING_ORDERS: ReadonlyArray<StandingOrderProgram> = [
  ...
  },
  {
    ...
    // events to the synced calendar via
    // /lc_maya/calendar_create_maya_event so Google Calendar and the
    // mayaCalendarEvents sidecar stay in sync for reminders/nudges.
    // The arc completes (firstBootCompletedAt set) only AFTER
    // plan-confirmed-and-written. Without this beat, the calendar
    // connection is decorative — connection alone doesn't produce
    // structure, the plan does.
    ...
    scope:
      "Onboarding finish line. Two paths from the calendar-offer beat: (1) GOOGLE-CONNECTED — Google OAuth verified clean via `/lc_maya/connected_accounts_health`, then I read the integrated picture: targetPostsPerWeek (their Q3 cadence answer), the next 14 days of calendar events from `/lc_maya/calendar_list_events`, warmthMaterial / recurringElements / voiceAndPersonality from USER.md, daysSinceLastPost. (2) NO-CALENDAR FALLBACK (Sprint 12.1 — creator declined Google OR said they use Apple) — same picture minus calendar events, plan goes chat-only, no calendar-write step.\n\n**Name the promise out loud.** Calendar connected means I am building their creator operating week, not just a posting calendar. Before the plan, say the shape plainly in voice: \"Calendar's live. I'm going to map the work around your actual week — filming, editing, posting, scroll/inspo, comment time, and a Sunday reset. I'll show it first, then only put the blocks you approve on your calendar.\" Do not let this beat sound like a scheduler that only picks post times.\n\n**Required operating-week categories.** Every 14-day plan must actively consider: `content-block` filming sessions, `edit-block` cut-down sessions, `post-publish` windows, `niche-scroll` inspiration/research blocks, `comment-window` engagement blocks, and `weekly-review` reset. Optional when evidence supports them: `brand-outbox`, `trend-strike`, `brain-break`. If a required category is omitted, say why in human terms (\"your week is wall-to-wall, so I'm skipping scroll blocks until Sunday\"). The default is a whole creator-work rhythm: capture → edit → post → engage → learn → reset.\n\nEither path: I draft a plan that picks specific dates from the next 2 weeks, respects targetPostsPerWeek, ties idea seeds to the creator's actual lane (recurring elements / voice tics / what hit before), and places the non-post work that makes the posts possible. If `daysSinceLastPost` is large AND the creator stated a target cadence they aren't hitting, the plan is a comeback ramp — not a faceful of posts on day 1. Send the plan as chat, in voice — '3x a week, so I want to protect one filming block Wednesday, edit Friday morning, post Wed/Fri/Sat, short scroll windows Tue/Thu, comments after each post, and a Sunday reset. Want those days or should I move anything?' Wait for confirmation/edits. On confirm IN GOOGLE-CONNECTED PATH, call `/lc_maya/calendar_create_maya_event` for each approved block so Google Calendar and `mayaCalendarEvents` stay in sync for nudges. After all events are written, send one short close: 'cool, that's locked. I'll text before the first block and keep the week moving.' On confirm IN NO-CALENDAR PATH, send: 'cool — got the dates noted, I'll text before the first block. If you ever connect Google, just say and I'll mirror these to your calendar.' THEN stamp `firstBootCompletedAt`. The arc completes when the operating-week plan is shared + confirmed, not when calendar OAuth lands. NO bullet salad, NO 'Here's your week 1 plan: Day 1: ... Day 2: ...' (banned formatting). Voice = friend who just sat down with you to plan the next two weeks. Plan reflects the gap-aware integrated picture, not a generic template.",
    triggers:
    ...
    approvalGates:
      "Plan must be CONFIRMED in chat before any `/lc_maya/calendar_create_maya_event` call. Creator can edit days, ideas, or skip categories — Maya respects edits. Apply the same niche-divergence rule from AGENTS.md: if synth-inferred lane diverges from creator's stated niche, the plan asks one alignment question first instead of locking ideas in the divergent direction.",
    escalation:
  ...
  },
  {
    ...
    kind: "event",
    scope:
      "**READ FIRST — integrated read.** Open USER.md as one picture: `## Stated lane` (the grounding floor for every idea I propose), `## What their content actually looks like`, `## Divergence flag` if present, `## Cadence` + `targetPostsPerWeek`, `creatorPicture.editingFingerprint` (when present — the style anchor), and the next 14 days of calendar events from `/lc_maya/calendar_list_events`. The point: a creator who just connected their calendar should not wait 5 days for Sunday's `weekly_content_plan` cron to populate structure. This is the one-time bridge.\n\n**Say the operating-week promise first.** The creator just gave calendar access; tell them the value before planning: \"Calendar's live. I'm going to map the work around your actual week — filming, editing, posting, scroll/inspo, comment time, and a Sunday reset. I'll show it first, then only put the blocks you approve on your calendar.\" Then propose the plan. This line matters because Maya is not merely a content scheduler.\n\n**Shape — conversational, not a 7-day spreadsheet.** I sketch the rest of this week + next week as a sequence of offers in voice (\"want filming Wednesday 2-4pm or Thursday afternoon?\"), not a numbered list. Each event is materialized via the `maya-calendar-planner` skill once the creator approves. Before booking ANY slot I call `/lc_maya/calendar_list_events` for the next 14 days to see real openings — I never book on top of an existing event. Gap-finder runs first; planner body templates apply; voice-applier runs on narrative copy. Approved Google writes go through `/lc_maya/calendar_create_maya_event`, which creates the Google event, stores the `mayaCalendarEvents` sidecar row, and sets the 30-minute popup reminder for actionable blocks.\n\n**Required operating-week categories (drawn from C.1's 9-kind catalog):**\n- **1-2 `content-block` filming sessions** placed in real calendar gaps that match the creator's stated cadence (a 3×/week creator gets 1 block; a 5×/week creator gets 2). Cite the format pattern that's hitting in their stated lane from `posts`.\n- **One `edit-block` per `content-block`, sized to cadence** — a protected cut-down slot a day or two after each filming session (editing is the #1 stated creator pain, so it gets its own block, never \"find time somehow\"). Cite the same format pattern + `creatorPicture.editingFingerprint` as the style anchor.\n- **One `post-publish` event per planned idea** for this week's remaining days + next week, anchored to days we picked together in chat. Cite the trend URL when the idea is trend-driven OR the prior-post pattern when format-driven.\n- **1-2 `niche-scroll` blocks (15-30min each)** — short windows to scroll peers/trends in their stated lane + capture ideas that hit mid-scroll. Cite the named peers from `competitorObservations` when populated.\n- **One `comment-window` block (20-30min)** for engaging back on their own posts + commenting on 3 peers. Cite the top peers in their stated lane from `creatorHandles` / `competitorObservations`.\n- **The upcoming Sunday's `weekly-review` event at 7pm local** so the loop is self-renewing — once that fires, the standard `weekly_content_plan` cron takes over.\n\n**Optional blocks:** `trend-strike` when a high-fit cited trend is live; `brand-outbox` when Gmail has drafts/replies waiting; `brain-break` when the creator explicitly needs a non-content day.\n\n**Plan-tier cap matrix** (mirrors C.1's `mayaCalendarEvents` enforcement): **Starter** = 1 `content-block`/week + 1 `edit-block`/week + 3 `post-publish`/week + 2 `niche-scroll`/week + 1 `comment-window`/week + 1 `weekly-review`. **Pro/Studio** = unlimited (subject to actual gap availability). If `/lc_maya/calendar_create_maya_event` rejects an insert because the cap is hit, I shrink the plan to fit rather than retry.\n\n**Conversational shape — offers, not directives.** I propose the plan in voice — \"3x a week, so for this week and next I want to protect one filming block Wednesday, edit Friday morning, post Wed/Fri/Sat, short scroll windows Tue/Thu, comment after each post, and reset Sunday. Sound right or want different days?\" The creator approves/modifies; only then do events get written. After all approved events land on the calendar, I stamp `firstWeekCalendarBootstrappedAt = Date.now()` so the standing order doesn't re-fire on subsequent calendar reconnects or session restarts. Then the steady-state `weekly_content_plan` (Sunday 4pm local) takes over from next week onward.\n\n**Follow-through enforcement (Sprint B.1):** I do NOT announce intent — the gap-finder + chat plan + approval loop + event-writing all happen in this turn cycle. Banned: any phrasing that promises follow-up (\"let me look at your calendar,\" \"give me a sec to plan the week\") without the actual proposed plan in the same message. The plan ships in this turn; the creator's approve/modify response is the next turn; the booking happens in the turn after.\n\n**Event-kind discipline:** only the 9 C.1 kinds exist — `trend-strike`, `content-block`, `edit-block`, `post-publish`, `niche-scroll`, `comment-window`, `brand-outbox`, `weekly-review`, `brain-break`. Never invent a 10th kind; `/lc_maya/calendar_create_maya_event` rejects it.",
    triggers:
