...
2. Is it above THEIR normal, or just a big account? account_posts on the author, then compare the candidate's views to that median. The ratio you were given is a start; the author's last twelve posts are the truth.
3. What are people reacting to? post_comments, only when it would change the version you write (a bit that landed, a question everyone asks). Not for Instagram unless it decides the pick.
4. Is this shape a wave this week? search_keyword with the shape's plainest keyword; three or more accounts doing it this week means "get in now or skip", one account means "their thing".
...
2. post_transcript on the top two outliers if you don't already have them; the words are the format.
