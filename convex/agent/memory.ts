...
export const byIds = internalQuery({
  ...
  handler: async (ctx, a): Promise<Array<{ kind: string; refId: string; text: string; at: number }>> => {
    const out: Array<{ kind: string; refId: string; text: string; at: number }> = [];
    for (const id of a.ids) {
      const m = (await ctx.db.get(id)) as Doc<"memories"> | null;
      if (m && m.creatorId === a.creatorId) out.push({ kind: m.kind, refId: m.refId, text: m.text, at: m.at }); // never another creator's, even by id
    }
    return out;
  },
});
...
export const recall = internalAction({
  ...
  handler: async (ctx, a): Promise<Array<{ kind: string; refId: string; text: string; at: number; score: number }>> => {
