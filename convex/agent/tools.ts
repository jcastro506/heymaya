...
export const TOOLS: OpenRouterTool[] = [
  ...
  { type: "function", function: { name: "lane_brands", description: "Brands seen paying creators in their lane in the last 30 days (paid or #ad posts from the accounts you watch for them), with how many creators and posts. Free. Start here before any brand research: a brand already paying people like them is a real lead; which one fits them is your call.", parameters: { type: "object", properties: { why: str }, required: ["why"] } } },
...
async function runToolInner(ctx: ActionCtx, creatorId: Id<"creators">, call: { name: string; args: Record<string, unknown> }, budget: ToolBudget, trace: ToolCallRecord[], sourceMessageId?: Id<"messages">): Promise<string> {
  ...
  try {
    ...
    if (call.name === "media_kit") {
      ...
      return cap(`lane: ${k.lane ?? "not confirmed"}\n${k.platforms.map((p) => `${p.platform}${p.handle ? ` @${p.handle}` : ""}: ${p.followers !== null ? `${p.followers.toLocaleString()} followers` : "followers unknown"} · normal ${p.normalViews !== null ? `${Math.round(p.normalViews).toLocaleString()} views` : "not settled yet"} · ${p.posts} posts read\n${p.best.map((b) => `  - ${b.views.toLocaleString()} views${b.multiple ? ` (${b.multiple}x)` : ""} · "${b.caption}" · ${b.url}`).join("\n")}`).join("\n")}\nprefs: ${k.prefs.paidOnly ? "paid only" : "open to gifting/affiliate"}; deal types ${k.prefs.dealTypes.join(", ") || "any"}; excluded ${k.prefs.excludedBrands.join(", ") || "none"}; minimum rate ${k.prefs.minimumRate}; region ${k.prefs.region}`);
    }
    if (call.name === "lane_brands") {
      const brands = await ctx.runQuery(internal.partnerships.signals.laneBrands, { creatorId });
      record(true, 0);
      return brands.length ? cap(brands.map((b) => `@${b.handle} (${b.platform}) · paid ${b.creators.length} creator${b.creators.length === 1 ? "" : "s"} in their lane (${b.creators.map((c) => `@${c}`).join(", ")}) · ${b.posts} post${b.posts === 1 ? "" : "s"}`).join("\n")) : "no paid posts seen in their lane in the last 30 days";
    }
