...
async function runToolInner(ctx: ActionCtx, creatorId: Id<"creators">, call: { name: string; args: Record<string, unknown> }, budget: ToolBudget, trace: ToolCallRecord[], sourceMessageId?: Id<"messages">): Promise<string> {
  ...
  try {
    ...
    if (call.name === "audience") {
      const facts = await ctx.runQuery(internal.connections.audience.forCreator, { creatorId });
      record(true, 0);
      if (!facts.length) return "no accounts on file for them yet";
      // Choreography rides in the response: nothing connected means the only honest next step is connecting.
    ...
    if (call.name === "watch_account") {
      ...
      if (!/^[a-z0-9._]{2,30}$/.test(handle)) { record(false, 0, "bad handle"); return "that isn't a handle i can watch"; }
      const r = call.args.act === "stop"
        ? await ctx.runMutation(internal.agent.manage.stopWatchingNow, { creatorId, handle })
        : await ctx.runMutation(internal.agent.manage.addAdmired, { creatorId, platform: call.args.platform === "instagram" ? "instagram" : "tiktok", handle });
      record(r.ok, 0);
      return r.ok ? `done: ${r.body}` : `not done: ${r.body}`;
    }
    ...
    if (call.name === "media_kit_link") {
      ...
      record(true, 0);
      return r.url ? `their media kit page is live: ${r.url} (public numbers only; they can turn it off any time)` : "their media kit page is off; the old link no longer works";
    }
