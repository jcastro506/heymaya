...
describe("one post, one row, whoever sees it first (live 2026-09-05)", () => {
  it("a row the connected feed created is claimed by the scrape, keeps its numbers, and is a photo without a duration", async () => {
    ...
    const after = await t.run(async (ctx) => await ctx.db.query("ownPosts").collect());
    expect(after.length, "the scrape must join the connected row by URL, not create a twin").toBe(1);
    expect(after[0].postId).toBe("3712345678901234567");
    expect(after[0].hashtags).toEqual(["tea", "matcha"]);
    expect(after[0].connected?.reach, "connected numbers survive the join").toBe(3);
  });
});
