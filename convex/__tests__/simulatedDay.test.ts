...
describe("a simulated day", () => {
  it("breakout → scout → idea and message rows → verdict → cap → reaction → taste", async () => {
    ...
    // 2026-09-08: the image skill is off her belt; no "show me", no render job.
    ...
    expect((await t.run((ctx) => ctx.db.query("jobs").collect())).some((j) => j.kind === "render_frames")).toBe(false);
