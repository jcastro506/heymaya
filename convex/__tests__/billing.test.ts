...
describe("billing.checkout.createCheckoutSession", () => {
  /**
   * NEW CONTRACT (post-2026-05-04 coach/manager rewrite, pending Agent 3
   * source patch): the 7-day trial applies to BOTH tiers on the FIRST
   * subscription only. The trial-day assertion is currently `it.skip`'d
   * and will activate as soon as Agent 3 lands the source change in
   * `convex/billing/checkout.ts` (rename `MANAGER_TRIAL_DAYS = 14` ->
   * `FIRST_SUB_TRIAL_DAYS = 7`, drop the `args.tier === "manager"` gate).
   * Per testing instructions, the failing assertion is preserved as a
   * spec for Agent 3 — DO NOT delete or weaken it.
   */
  const EXPECTED_TRIAL_DAYS = 7;

  it.skip.each<["coach" | "manager", "monthly" | "annual", string]>([
    ["coach", "monthly", "price_coach_m"],
    ["coach", "annual", "price_coach_a"],
    ["manager", "monthly", "price_manager_m"],
    ["manager", "annual", "price_manager_a"],
  ])(
    "PLAN-TIER × ACTION (PENDING Agent 3 source patch): fresh creator picks %s/%s -> price=%s + 7-day trial",
