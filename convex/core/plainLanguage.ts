...
const MACHINE_PATTERNS: Array<{ name: string; re: RegExp }> = [
  ...
  {
    name: "id",
    re: /(?:,?\s*\b(?:draft|post|placement|asset|message|creator)?\s*ID:?\s*)?[([]?`?\b[a-z0-9]{32}\b`?[)\]]?/gi,
  },
  /**
...
export function checkPlainLanguage(
   ...
   *
   * ⚠️ FOUND LIVE, 2026-08-21. The creator's product is creatify.ai, and
   * "Creatify" is on the denylist below because Creatify is OUR VIDEO VENDOR.
   * So this function quietly deleted the creator's own product name from every
