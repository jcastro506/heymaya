...
export interface UnreachableCreator {
...
/**
 * Group undelivered outbound messages by creator.
 *
...
export const fleetUnreachable = internalQuery({
  ...
  handler: async (ctx): Promise<UnreachableCreator[]> => {
    ...
    const byCreator = new Map<Id<"creators">, UnreachableCreator>();
    ...
    for (const message of [...pending, ...inbound]) {
      ...
      const existing = byCreator.get(message.creatorId);
      if (!existing) {
        byCreator.set(message.creatorId, {
    ...
    return [...byCreator.values()].sort((a, b) => a.since - b.since);
  },
});
