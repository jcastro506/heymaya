...
export const claimNext = internalMutation({
  ...
  handler: async (ctx, args): Promise<Doc<"jobs"> | null> => {
     ...
     *
     * So the creator with the FEWEST jobs already running goes next. A burst
     * from one account interleaves with everyone else instead of blocking them,
     * and an idle creator is never behind a busy one.
     */
...
export const deadLetters = internalQuery({
  args: { creatorId: v.optional(v.id("creators")), limit: v.optional(v.number()) },
  handler: async (ctx, args): Promise<Doc<"jobs">[]> => {
    const rows = await ctx.db
      .query("jobs")
      .withIndex("by_status_and_runAfter", (q) => q.eq("status", "dead"))
      .take(args.limit ?? 100);
    if (!args.creatorId) return rows;
    return rows.filter((row) => row.creatorId === args.creatorId);
  },
});
