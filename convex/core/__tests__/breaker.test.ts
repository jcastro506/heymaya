...
async function reading(
  ...
  await t.mutation(internal.core.breaker.record, {
    vendor: "scrapecreators",
    verdict,
    balance: verdict === "critical" ? 12 : 3000,
    detail: `scrapecreators is at 12 credits — every creator stops when this hits zero`,
    now: at,
  });
}
...
describe("⚠️ IT FAILS OPEN, AND THAT IS THE POINT", () => {
  it("never checked means GO", async () => {
    ...
    const res = await t.query(internal.core.breaker.vendorOpen, {
      vendor: "scrapecreators",
      now: NOW,
    });
...
describe("what actually trips it", () => {
  ...
  it("one row per vendor, however many readings", async () => {
    ...
    const all = await t.query(internal.core.breaker.allVendors, {});
