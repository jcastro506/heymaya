...
async function seed(t: ReturnType<typeof convexTest>): Promise<Id<"creators">> {
  return await t.run(async (ctx) => seedCreator(ctx, "one", { timezone: "UTC" }));
}

describe("⚠️ THE DENYLIST MUST NOT EAT THE FOUNDER'S OWN PRODUCT NAME", () => {
  /**
   * Found live 2026-08-21. The creator's product is creatify.ai; "Creatify" is
