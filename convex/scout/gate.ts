...
export const setInvestigation = internalMutation({
  args: { signalIds: v.array(v.id("signals")), trace: v.array(v.object({ tool: v.string(), params: v.any(), why: v.string(), credits: v.optional(v.number()), ms: v.number(), ok: v.boolean(), detail: v.optional(v.string()), result: v.optional(v.string()) })) },
  handler: async (ctx, a): Promise<null> => {
