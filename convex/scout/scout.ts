...
Output ONLY JSON:
{"pick": {"postId": "", "notable": true, "fit": "yes|maybe|no", "fitWhy": "≤160, written TO them in your voice (you, your), because they read it in their app under \"why it's for you\": e.g. \"your deadpan post-run clips are your best format, and this is one\"", "transfer": false, "newForYou": false, "features": {"format": "talking-head|skit|vlog|tutorial|list|reaction|duet|pov|grwm|text-on-screen|other", "topics": ["≤3 short tags"], "tone": "serious|deadpan|ironic|hype|warm", "lengthBucket": "<15|15-30|30-60|60+", "sound": "trending|original|none"}, "message": "≤900 chars, in your voice, lowercase fine, no bullets", "version": {"hook": "≤120", "onScreenText": "≤80", "lengthSec": 0, "sound": "≤80 or ''", "block": {"startLocal": "YYYY-MM-DDTHH:MM on their clock", "lengthMin": 60, "title": "≤60, starts with 'film:'"} | null}} | null,
 "rejected": [{"postId": "", "why": "≤80, must say whether the topic or the format was the problem"}], "forYou": {"postId": "", "line": "≤160: one line, no ask, why you thought of them"} | null}` + LOOKUPS.scout;
...
export const run = internalAction({
  ...
  handler: async (ctx, args): Promise<{ sent: boolean; reason: string; dry?: { message: string; pick: unknown; rejected?: unknown; evidence: unknown; trace: unknown } }> => {
    ...
    const evidence: Array<{ postId: string; kind: string; url: string; ratio: number | null; why: string; transcript: string | null; handle: string | null; taste: string; pack?: Record<string, unknown> }> = [];
    for (const s of g.candidates) {
      ...
      // B2 after-a-hit: a win carries the same evidence pack explain-post uses, so any "why" is grounded.
      const pack = s.kind === "win" ? await ctx.runQuery(internal.agent.opinion.packForPostId, { creatorId: args.creatorId, postId: s.sourcePostIds[0] }) : null;
      evidence.push({ postId: s.sourcePostIds[0], kind: s.kind, url, ratio: s.kind === "shape" ? null : s.score, why: s.detected ?? s.why, transcript, handle, taste: g.tasteHints[s._id] ?? "", ...(pack ? { pack: pack.facts } : {}) });
    }
    ...
    let parsed: { pick: null | { postId: string; notable: boolean; fit: "yes" | "maybe" | "no"; fitWhy: string; message: string; version: unknown; newForYou?: boolean; visual?: boolean; features?: { format?: string; topics?: string[]; tone?: string; lengthBucket?: string; sound?: string } }; rejected?: Array<{ postId: string; why: string }>; forYou?: { postId: string; line: string } | null };
    try {
    ...
    if (!pick || !pick.message?.trim()) {
      ...
      // §24: a day with no idea can still carry "saw this, thought of you", once or twice a week, no ask.
      const fy = parsed.forYou;
      const fySignal = fy ? g.candidates.find((s) => s.sourcePostIds.includes(fy.postId)) : undefined;
      const fyUrl = fySignal ? (fySignal.why.match(/https?:\/\/\S+/)?.[0] ?? null) : null;
      if (fy && fyUrl) await ctx.runAction(internal.agent.cadence.forYou, { creatorId: args.creatorId, postId: fy.postId, url: fyUrl, line: fy.line, now });
     ...
     * the critic failed the linkless draft for no_link, the rewrite was linkless too, and the
    ...
    // A breakout, a rising sound, a shape of the week or a calendar event goes stale in days; a win or a worth-seeing can wait.
    const urgency: "now" | "any" = signal.kind === "breakout" || signal.kind === "sound" || signal.kind === "shape" || signal.kind === "calendar" ? "now" : "any";
    const ideaId = await ctx.runMutation(internal.scout.scout.writeIdea, { creatorId: args.creatorId, signalId: signal._id, urgency, evidenceLinks: links, fit: pick.fit, fitWhy: pick.fitWhy, version: pick.version ?? {}, messageText: pick.message, produced, features, newForYou: Boolean(pick.newForYou) && g.exploreOpen });
