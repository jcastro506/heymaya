...
export const sendDraft = internalAction({
  ...
  handler: async (ctx, a): Promise<{ messageId: Id<"messages"> }> => {
    ...
    const info = await ctx.runAction(internal.reads.read.read, { kind: "post.info", params: { platform, url: a.url }, creatorId: a.creatorId });
