...
function toolsMd(): string {
  return [
    ...
    "- media.catalog_creator_asset",
    "- media.search_creator_assets",
    "- media.get_creator_asset",
    ...
    "- media.record_tiktok_draft_handoff",
    ...
    "TikTok publishing tools are draft-handoff only. Maya must not direct-post to TikTok; she can send a download/iMessage handoff or use TikTok upload-to-inbox after OAuth/app approval.",
...
function creatorMayaSkills(): Array<{ slug: string; body: string }> {
  const skills = [
    ...
    },
    {
      ...
      body: skillMd({
        ...
        process: [
          ...
          "If the asset is already cataloged at the current analysis version, reuse the stored catalog; do not make Maya re-watch/re-analyze the same bytes.",
          "For images, catalog with a multimodal model. For videos, catalog with transcript, sampled frames, scene summary, OCR, music cue, and creator-relevance tags.",
          ...
          "Persist the analysis through media.catalog_creator_asset: primary subject, scene summary, detected text, transcript, retrieval tags, visual quality, safety concerns, and likely TikTok uses.",
          ...
          "When the creator asks for older footage, use media.search_creator_assets first, then media.get_creator_asset for exact durable URLs.",
        ...
        gates: [
          ...
          "Never re-watch an unchanged asset unless the catalog is missing, stale, or the creator asks for a deeper read.",
        ],
      }),
    },
    {
    ...
    {
      slug: "creator-tiktok-draft-handoff",
      body: skillMd({
        title: "Creator TikTok Draft Handoff",
        description:
          "Hand completed TikTok edits back to the creator without auto-posting.",
        useWhen:
          "Use after Maya has a rendered TikTok-ready asset or the creator asks to send/save/open the final edit in TikTok.",
        inputs: [
          "creator id and active iMessage channel",
          "creatorMayaV0EditRequests row",
          "rendered media asset id and durable URL",
          "caption draft, hashtags, suggested music, and posting checklist",
          "TikTok OAuth/upload-to-inbox availability when configured",
        ],
        process: [
          "Confirm the edit request has a rendered asset id before handoff.",
          "Prefer TikTok upload-to-inbox only when Convex confirms TikTok OAuth, required scopes, and app approval; otherwise use iMessage/download handoff.",
          "Send the creator explicit copy: Maya exported the edit and did not post it.",
          "Provide caption, hashtags, suggested music/sound URL when available, thumbnail frame, and final TikTok checklist.",
          "Record the handoff through media.record_tiktok_draft_handoff with mode, status, caption, suggested music, instructions, and provider ids when present.",
          "Ask the creator to confirm after posting if they want Maya to monitor performance.",
        ],
        tools: [
          "media.get_creator_asset",
          "media.record_tiktok_draft_handoff",
        ...
        gates: [
          "Never use TikTok Direct Post in v0.",
          "Never say the asset is in drafts unless TikTok upload-to-inbox returned a provider confirmation.",
          "Never bake commercial music into the rendered file unless rights are confirmed; recommend music for the creator to add in TikTok.",
          "Never mark creator_posted from inference; only record it from creator confirmation or TikTok evidence.",
        ],
      }),
    },
    {
