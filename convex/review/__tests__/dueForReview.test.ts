...
describe("no review before there is a week to review (live 2026-09-06)", () => {
  it("a creator paired this morning is not due; one paired ten days ago is", async () => {
    ...
    // Seeded, not patched in `t.run`: a raw patch skips the trigger that keeps the schedule row true.
    const fresh = await t.run((ctx) => seedCreator(ctx, "a", { timezone: "America/New_York", channel: { paired: true }, createdAt: SUNDAY_10AM_NY - 3 * 3_600_000 }));
    const old = await t.run((ctx) => seedCreator(ctx, "b", { timezone: "America/New_York", channel: { paired: true }, createdAt: SUNDAY_10AM_NY - 10 * 86_400_000 }));
