...
export const PARTNERSHIP_TOOLS: OpenRouterTool[] = [
  ...
  { type: "function", function: { name: "partnership_research", description: "Search public brand/program/category terms (query), OR extract one public official page (url), OR read a brand's official TikTok/Instagram profile bio (profile: \"instagram:handle\" or \"tiktok:handle\"; its email counts as a contact only when the brand's official site links that account). Do not include private creator information. Bounded monthly allowance. Use official program pages to find preferred routes, requirements and published emails. A result is not proof of eligibility or deliverability.", parameters: { type: "object", properties: { query: str, url: str, profile: str }, required: [] } } },
  ...
  { type: "function", function: { name: "partnership_draft", description: "Prepare and separately show an exact email review, copyable DM, or application answers. Requires opportunityId, subject and body. Uses the saved preferred route and recipient, never model-supplied addresses. No attachments in this version. For forms, draft only visible questions and clearly mark missing answers; user submits. For an application route also pass answers: a JSON array [{label, answer}], one per saved applicationFields label (exact label text), which the app shows field by field with a Copy button; leave out a question you can't answer from their records and say so in the body. For email the user must type the exact SEND code in the separately delivered review. This tool NEVER sends to a brand. Edits create a new revision and invalidate prior approval.", parameters: { type: "object", properties: { opportunityId: str, subject: str, body: str, answers: str }, required: ["opportunityId", "subject", "body"] } } },
...
export async function runPartnershipTool(ctx: ActionCtx, creatorId: Id<"creators">, name: string, args: Record<string, unknown>, sourceMessageId?: Id<"messages">): Promise<string> {
  ...
  if (name === "partnership_read") result = await ctx.runQuery(internal.partnerships.store.read, { creatorId, brandDomain: typeof args.brandDomain === "string" ? args.brandDomain : undefined, cursor: typeof args.cursor === "string" ? args.cursor : undefined, ...(args.opportunityId ? { opportunityId: String(args.opportunityId) as Id<"partnershipOpportunities"> } : {}) });
    ...
    if (name === "partnership_research") result = await ctx.runAction(internal.partnerships.research.run, { creatorId, query: typeof args.query === "string" ? args.query : undefined, url: typeof args.url === "string" ? args.url : undefined, profile: typeof args.profile === "string" ? args.profile : undefined });
    ...
    } else if (name === "partnership_draft") {
      // 2026-09-24 (deals world): the tool never carried answers, so the app's field-by-field Copy screen was always empty from chat.
      const answers = typeof args.answers === "string" && args.answers.trim() ? JSON.parse(args.answers) : Array.isArray(args.answers) ? args.answers : undefined;
      result = await ctx.runMutation(internal.partnerships.drafts.prepare, { creatorId, sourceMessageId, input: { opportunityId: args.opportunityId, subject: args.subject, body: args.body, ...(answers ? { answers } : {}) } });
    }
