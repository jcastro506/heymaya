...
/* -------------------------------------------------------------------------- */
/* Skill dependency wiring                                                     */
/* -------------------------------------------------------------------------- */

/**
 * The variant drafter is a thin adapter over an async function that, given a
 * fully-built prompt, returns a JSON string the orchestrator can parse into
 * the 4 reply variants. Real production wiring routes through
 * `internal.agents.modelRouter.maya.callMaya` with task tag
 * `brand_email_draft` (high thinking). Tests inject a fixture adapter that
 * returns deterministic JSON without invoking the model-router.
 *
 * The orchestrator builds the prompt via the skill's `buildVariantPrompt`
 * helper (`agents/skills/maya-brand-deal-triager/script.ts`); this enforces
 * Sprint 8 Slice A's zero-hardcoded-prose rule — every word the creator sees
 * is generated by the model, not templated by the orchestrator.
 */
export type VariantPromptAdapter = (prompt: string) => Promise<string>;

/**
 * Parse the variant drafter's JSON output. Lenient about surrounding
 * whitespace and markdown code-fences. Returns empty strings for any
 * variant the model omitted — the skill's per-variant skip-on-empty guard
 * drops those cleanly.
 */
function parseVariantJson(raw: string): Record<ReplyVariantKind, string> {
  const cleaned = raw.trim().replace(/^```(?:json)?/i, "").replace(/```$/i, "").trim();
  let parsed: unknown;
  try {
    parsed = JSON.parse(cleaned);
  } catch {
    // Model emitted non-JSON. Skip all variants — caller's per-variant skip
    // guard handles the empty-string case.
    return { enthusiastic: "", neutral: "", firm: "", pass: "" };
  }
  ...
  return {
    enthusiastic: pick("enthusiastic"),
...
/**
 * Skill dependency wiring for the brand-deal triager.
 *
 * Sprint 8 Slice A removed the previously-hardcoded fallback variant prose.
 * The orchestrator no longer synthesizes user-facing prose; it builds a
 * prompt via `buildVariantPrompt` and routes through the supplied
 * `variantPromptAdapter`. In production the adapter calls
 * `internal.agents.modelRouter.maya.callMaya`; in tests the adapter returns
 * deterministic JSON. Either way, the orchestrator owns no prose.
 *
 * Other deps remain deterministic stubs in v0 — those are non-prose
 * decisions (numbers, classifications, pass/fail flags) and intentionally
 * out of scope for the prose-deletion slice. Real wiring lands in a later
 * sprint per `agents/skills/maya-brand-deal-triager/SKILL.md` § calls.
 */
function buildDefaultTriageDeps(opts: {
  variantPromptAdapter: VariantPromptAdapter;
}): {
  ...
  return {
    rateCalculator: {
      async suggest({ floorRate }) {
        // Deterministic numeric stub — no user-facing prose, safe to keep
        // until the real comparable-lookup skill ships.
        return {
          low: floorRate,
          target: Math.round(floorRate * 1.4),
          stretch: Math.round(floorRate * 1.8),
          reasoning:
            "Stub reasoning — real comparable lookup ships when the rate-calculator skill is wired.",
        };
      },
    },
    contractRedflag: {
      async scan() {
        return {
          ...
          summary:
            "Stub — real PDF scan wires when the contract-redflag skill is wired.",
        };
      },
    },
    voiceApplier: {
    ...
    variantDrafter: {
      async draftVariants(input) {
        const prompt = buildVariantPrompt({
          thread: input.thread,
          classification: input.classification,
          parsedOffer: input.parsedOffer,
          suggestedRate: input.suggestedRate,
          voiceFingerprint: input.voiceFingerprint,
        });
        const raw = await opts.variantPromptAdapter(prompt);
        return parseVariantJson(raw);
      },
    },
  };
}
...
export const triageInboundEmail = internalAction({
  ...
  handler: async (
    ...
    // 3. Build deps + run the skill. The variant drafter routes through the
    //    model router (`internal.agents.modelRouter.maya.callMaya`) using the
    //    skill's `buildVariantPrompt` helper. The orchestrator owns no
    //    user-facing prose — every word the creator sees is generated by
    //    the model from the prompt the skill builds. (Sprint 8 Slice A.)
    //
    //    If the model call fails (auth, network, parse error), the per-
    //    variant skip-on-empty guard inside the skill drops the variant
    //    cleanly — the creator gets fewer drafts rather than fabricated
    //    prose. This is the operator-locked principle: "grounded or silent."
    const variantPromptAdapter: VariantPromptAdapter = async (prompt) => {
      try {
        const modelResult = await ctx.runAction(
            ...
            messages: [
              {
                role: "system",
                content:
                  "You draft brand-email reply variants for a creator. Mirror the creator's voice fingerprint exactly. Return strict JSON only — no commentary, no markdown.",
              },
        ...
        // Model unavailable — return empty JSON so the per-variant skip
        // guard drops every variant cleanly. Better fewer-and-real than
        // many-and-fictional.
        return '{"enthusiastic":"","neutral":"","firm":"","pass":""}';
      }
    };

    const deps = buildDefaultTriageDeps({ variantPromptAdapter });
    ...
    // 5. Threshold-driven action: in MVP we NEVER auto-send. The creator
    //    approves and sends manually from Gmail.
    //
    //    Sprint 7 Slice A carry-forward (Sprint 8 Slice A landing): the prior
    //    behavior called `gmailActions.sendEmail` directly when the offer
    //    cleared the floor. That bypassed `maya-gmail-draft`'s runtimeGuard
    //    and violated the operator's locked rule "NEVER auto-send email in
    //    MVP." We replaced sendEmail with `gmailActions.createDraft` so the
    //    creator approves + sends from Gmail.
    //
    //    The schema `connectedAccounts.autoSendThreshold` field is preserved
    //    intentionally — it backs future re-enablement. MVP does not honor
    //    it for sending; it only gates whether we PRE-DRAFT into Gmail's
    //    Drafts folder for under-threshold offers (so the creator can act in
    //    one tap from their phone). This gives the operator a path to flip
    //    the auto-send back on later via env flag without a schema migration.
    //
    //    `MVP_ALLOW_AUTO_SEND` (env, default unset) is the explicit operator
    //    gate. When unset (the default and the only MVP behavior), even the
    //    pre-draft path uses `createDraft`. When explicitly set to "true" by
    //    the operator post-MVP, the firm variant is sent directly. Tests
    //    assert the unset case never sends.
    let autoSent = false;
    ...
    const mvpAllowAutoSend = process.env.MVP_ALLOW_AUTO_SEND === "true";
    if (
      threshold !== undefined &&
      threshold !== null &&
      offerValue !== null &&
      ...
      try {
        ...
        if (mvpAllowAutoSend) {
          // Operator-gated post-MVP path. Default-off; only fires when
          // `MVP_ALLOW_AUTO_SEND=true` is explicitly set in the Convex env.
          ...
          await ctx.runMutation(internal.dealTriage.logActionRow, {
            ...
            outcome: "ran",
            detail: `offer=${offerValue} threshold=${threshold} variant=firm autoSend=true`,
          });
        } else {
          // MVP path — pre-draft into Gmail's Drafts folder. Creator opens
          // Gmail, reviews, and sends. No prose leaves Maya without human
          // approval. autoSent stays false.
          await gmailActions.createDraft(
            { connectedAccountId: decryptedAccountId },
            {
              to: args.gmailThread.from.email,
              subject: `Re: ${args.gmailThread.subject}`,
          ...
          await ctx.runMutation(internal.dealTriage.logActionRow, {
            creatorId: args.creatorId,
            entryId: "brand_email_triage_autodraft",
            outcome: "ran",
            detail: `offer=${offerValue} threshold=${threshold} variant=firm autoSend=false (MVP)`,
          });
        }
      } catch (err) {
        // Draft/send failed — log but don't crash. Creator still has the
        // draft variants in brandDeals to act on manually.
        await ctx.runMutation(internal.dealTriage.logActionRow, {
          creatorId: args.creatorId,
          entryId: mvpAllowAutoSend
            ? "brand_email_triage_autosend"
            : "brand_email_triage_autodraft",
          outcome: "failed",
          detail: (err as Error).message?.slice(0, 500) ?? "unknown",
        });
      }
