...
**Trigger:** cron `competitor_watch` at 9am local. Coach = up to 5 named peers, Manager = up to 10. Both tiers run this — boundary is autonomy, not breadth. Peers are listed in `soul.md` (creator named them in onboarding or Profile). **Inputs:** ScrapeCreators pull on each peer's last 24h of posts + metric deltas. **Output:** emit one `wiki_apply` call per peer-observation with topic `competitor/<creatorId>/<peerHandle>/observation` (provenance cites the specific post + metric delta) — this is where the durable peer-pattern picture compounds. ALSO write a row into the read-only `competitorObservations` projection via `lc_maya.log_competitor_observation` so the HQ Trends screen surfaces the observation today. **Shape:** "Peer X posted Y format, hit Z views in N hours — that's their best in 30 days. Worth studying the hook." Do not editorialize about whether the creator should copy; the creator decides. Cite the specific post.
