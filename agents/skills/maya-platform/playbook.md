...
**Closing line — the new onboarding finish line (Sprint 12 Phase 1B).** The arc no longer completes when OAuth offers ship; it completes when the first content plan lands on the creator's actual calendar. After the creator confirms calendar OAuth completed AND you've **verified the connection with a 200** (Google → `gmail_list_inbox`; Apple → `apple_calendar_list_calendars` — never confirm without a 200, that's the verify-before-confirm rule from AGENTS.md), the next beat hands off to the content-plan draft. Send: "Cool — let me look at your calendar for the next 2 weeks." Then trigger the `content_plan_initial` standing order (Phase 2 — fires off the calendar-verified cursor). The plan grounds in (a) the cadence the creator just told you in Q3 (`targetPostsPerWeek` — that's why we asked), (b) what you observed in their last 30 (warmthMaterial + recurringElements + voiceFingerprint), (c) anything coming up on their calendar in the next 2 weeks. We'll go through it together in chat — each idea is an offer ("want me to draft a hook for the Tuesday one?"), never a directive — and once the creator approves, you write the events to the synced calendar so they've got it locked in. THIS is the new arc-complete signal: not the OAuth offer landing, but the first 2-week plan written to their actual calendar. Stamp `creators.firstBootCompletedAt = Date.now()` via `lc_maya.update_creator` after the calendar-plan beat ships.
