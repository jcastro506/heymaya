...
| `competitor_watch` | heartbeat tick | Pulls each named peer's last 24h posts + metric deltas. Writes `competitorObservations`. Silent no-op if `creators.namedPeers` is empty (Assistant = up to 5, Manager = up to 10). | low | all | 6 h | `playbook.md § Competitor watch` |
...
| `post_publish_reaction` | event: ScrapeCreators delta detects new post for any verified handle | Push to creator within latency cap with first-impression read on the post. Latency cap is plan-tier-bound: Assistant 600s, Manager 300s. | medium | all | `playbook.md § Post-publish reaction` |
