...
| `first_proactive_ping` | event: `creators.pictureLockedAt` stamped AND `creators.firstProactivePingSentAt === undefined`. Convex scheduler fires the composer at `pictureLockedAt + uniformRandom(15min, 30min)`. | Day 1 first-touch ping: 1 cited trend + 1 grounded idea (≥2 post-id citations) + Gmail/Calendar connect offers, written to `firstProactivePings` for the agent heartbeat to send via claw-messenger. Stamps `creators.firstProactivePingSentAt` at compose time so the event cannot re-fire. | medium | all | empty-input precedence: BOTH empty → silent no-op (status='skipped'); ONE empty → ship with the leg that worked. Idempotency-guarded by `firstProactivePingSentAt`. The ping body passes a banned-phrase floor (anti-sycophancy) — if it trips, falls back to skip-empty. | `playbook.md § Day 1 first-proactive-ping` |
...
| `first_proactive_ping` (event) | Y | Y |
...
| `first_proactive_ping` | `playbook.md § Day 1 first-proactive-ping` | medium (explicit override on the standing-order entry; voice-critical first-impression content) | event |
